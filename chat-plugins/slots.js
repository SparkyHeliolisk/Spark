...
function display(result, user, slotOne, slotTwo, slotThree) {
	...
	if (!result) {
		display += 'Aww... bad luck, <b>' + EM.nameColor(user) + '</b>. Better luck next time!</font>';
	}
	if (result) {
		display += 'Congratulations, <b>' + EM.nameColor(user) + '</b>. You have won ' + slots[slotOne] + ' bucks!!</font>';
	}
	return display + '</div>';
}
